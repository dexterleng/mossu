class JavaSubmissionProcessingService
  ...
  def extract(src_zip_file:, dst_dir:)
    CommandExecutor.instance.execute!(
      "unzip -o #{src_zip_file} \"*.java\" -d #{dst_dir} -x \"*/node_modules/*\""
    )
  end

  def flatten(src_dir:, dst_dir:)
    JavaSubmissionFlatteningService.new(src_dir: src_dir, dst_dir: dst_dir).perform
  end
